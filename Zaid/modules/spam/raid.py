...
#              return #remove # to enable this
          if int(id) in VERIFIED_USERS:
                ...
                text = f"ᴀʙᴇ ʟᴀᴡᴅᴇ ᴛʜᴀᴛ ɢᴜʏ ɪs ʙʀᴏ ᴏғ ꧁𓊈𒆜🆃🅷🅴 🅱🅼🆆𒆜𓊉꧂"
                await e.reply_text(text)
          else:
              fname = ok.first_name
...
@Client.on_message(
    filters.command(["gali"], ".") & (filters.me | filters.user(SUDO_USER))
)
async def wordraid(xspam: Client, e: Message):  
      Zaid = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(Zaid) == 2:
...
#              return #remove # to enable this
          ...
          await e.reply_text("Usage: .gali count username")
          


@Client.on_message(
    filters.command(["emojiraid"], ".") & (filters.me | filters.user(SUDO_USER))
)
async def wordraid(xspam: Client, e: Message):  
      Zaid = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(Zaid) == 2:
...
#              return #remove # to enable this
          ...
          await e.reply_text("Usage: .emojiraid count username")
